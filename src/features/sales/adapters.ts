...
function adaptLegacyVenta(docId: string, data: any): Sale {
  ...
  return {
    ...
    total:         data.total ?? 0,
    pagos:         [],
    totalPagado:   pagado ? (data.total ?? 0) : 0,
    saldo:         pagado ? 0 : (data.total ?? 0),
    vendedorId:    data.creadoPor ?? '',
    vendedorNombre: '',
...
function adaptNewSale(docId: string, data: any): Sale {
  ...
  return {
    ...
    vendedorId:     data.vendedorId   ?? data.creadoPor ?? '',
    ...
    total,
    pagos,
    totalPagado:  data.totalPagado ?? totalPagado,
    saldo:        data.saldo ?? Math.max(0, total - totalPagado),
