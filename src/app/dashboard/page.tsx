...
import { Plus, Building2, ShoppingBag, Layers, ChevronRight, Settings, LogOut } from 'lucide-react'
import { useAuthStore, useWorkspaceStore } from '@/store'
import { getWorkspaces, createWorkspace, logOut } from '@/lib/services'
import type { Workspace, WorkspaceType } from '@/types'

const WORKSPACE_TYPES: { tipo: WorkspaceType; label: string; desc: string; emoji: string; color: string }[] = [
  { tipo: 'servicios', label: 'Servicios', desc: 'Alarmas, seguros, internet, telecomunicaciones', emoji: '🛡️', color: '#E8001D' },
  { tipo: 'productos', label: 'Productos', desc: 'Electrónica, ropa, accesorios, importación', emoji: '📦', color: '#2563eb' },
  { tipo: 'mixto',     label: 'Mixto',     desc: 'Combinación de productos y servicios', emoji: '⚡', color: '#7c3aed' },
]
...
export default function DashboardPage() {
  ...
  const { workspaces, setWorkspaces, setActiveWorkspace, activeWorkspaceId } = useWorkspaceStore()
  const [loading, setLoading] = useState(true)
  const [showNew, setShowNew] = useState(false)
  const [creating, setCreating] = useState(false)

  // Nuevo workspace form
    ...
    return (
      <div className="min-h-screen bg-surface-950 flex items-center justify-center">
        <div className="text-center">
          <div className="w-10 h-10 rounded-xl bg-brand-600 flex items-center justify-center mx-auto mb-3 animate-pulse">
        ...
        <div className="space-y-2 mb-4">
          {workspaces.map(ws => (
            <button
              ...
              className="w-full flex items-center gap-4 bg-surface-900 hover:bg-surface-800 border border-surface-800 hover:border-surface-700 rounded-2xl p-4 transition-all text-left group animate-slide-up"
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-surface-500 text-xs mt-0.5 capitalize">
                  {WORKSPACE_TYPES.find(t => t.tipo === ws.tipo)?.label} · {ws.config.moneda ?? 'ARS'}
                </div>
              </div>
              <ChevronRight size={16} className="text-surface-600 group-hover:text-surface-400 transition-colors flex-shrink-0" />
            </button>
          ))}
        </div>
          ...
          <div className="bg-surface-900 border border-surface-800 rounded-2xl p-5 animate-scale-in">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder="Ej: iPhone Club, Verisure, Mi Seguro..."
                autoFocus
            ...
            {/* Tipo */}
            <div className="mb-4">
              <label className="block text-xs font-medium text-surface-400 mb-2">Tipo de negocio</label>
              <div className="space-y-2">
                {WORKSPACE_TYPES.map(t => (
                  <button
                    key={t.tipo}
                    onClick={() => setNewTipo(t.tipo)}
        ...
        {workspaces.length === 0 && !showNew && (
          <div className="text-center mt-12">
            <p className="text-surface-600 text-sm">
              Todavía no tenés ningún negocio.<br/>Creá el primero para empezar.
            </p>
          </div>
        )}
      </div>
